...
export function ConcernsScreen({ onNext }: ConcernsScreenProps) {
  ...
  const handleNext = () => {
    if (selectedConcerns.size > 0) {
      const concernsArray = Array.from(selectedConcerns).filter(
        (c) => c !== "none",
      );
      ...
      const concernTips: Record<string, string> = {
        ...
        hyperpigmentation:
          "Face yoga boosts circulation to fade dark spots naturally over time.",
        rosacea:
          "Ultra-gentle movements reduce inflammation. Avoid heat-inducing techniques.",
        "fine-lines":
          "Targeted exercises strengthen underlying muscles to plump fine lines.",
        sagging:
          "Consistent lifting movements combat gravity and restore facial contours.",
        texture:
          "Regular massage and stimulation smoothens bumpy, uneven skin texture.",
      ...
      const phaseData = {
        ...
        tips: selectedList
          .map((concern) => concernTips[concern])
          .filter(Boolean)
          .slice(0, 3),
      };
